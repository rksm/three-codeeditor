;(function(exports) {
  ...
  function drawTextLayer(ctx, ed, scrollLeft) {
    ...
    var r           = ed.renderer;
    var screenPos   = ed.renderer.textToScreenCoordinates(ed.renderer.getFirstVisibleRow(), 0);
    var bounds      = ed.renderer.container.getBoundingClientRect();
    var offset      = 0;
    var lineHeight  = ed.renderer.layerConfig.lineHeight
    ...
    var lineElements = r.$textLayer.element.childNodes,
    ...
    for (var i = 0; i < lineElements.length; i++) {
        ...
        for (var j = 0; j < tokenEls.length; j++) {
            ...
            if (measured) {
              ...
              ctx.fillText(text, localCoords.x, localCoords.y);
  ...
  function drawMarkerLayer(ctx, ed, scrollLeft, markerLayer) {
    ...
    for (var key in markerLayer.markers) {
        ...
        if (marker.renderer && false) {
            ...
            console.warn("drawFullLineMarker not yet implemented");
            // markerLayer.drawFullLineMarker(ctx, markerLayer, scrollLeft, marker, range, marker.clazz, config);
