;(function(exports) {
  ...
  function onAceEditorAfterRenderEvent(aceEditor, codeEditor) {
    ...
    var editorStyle = window.getComputedStyle(containerEl);
    ...
    function copyText(codeEditorAce, codeEditorCanvas) {
      ...
      var fontStyle  = window.getComputedStyle(r.$fontMetrics.$measureNode);
      c.font     = fontStyle.font;
      ...
      for (var i = 0; i < lineElements.length; i++) {
          ...
          for (var j = 0; j < tokenEls.length; j++) {
              ...
              var cssDecl = window.getComputedStyle(tokenEl);
              if (cssDecl) {
                c.font = cssDecl.font;
    ...
    function drawCursorLayer(ed, cursorLayer) {
      ...
      for (var i = 0, n = selections.length; i < n; i++) {
          ...
          var style = window.getComputedStyle(cursorEls[i]);
          ...
          c.fillRect(pixelPos.left,pixelPos.top,width,config.lineHeight);
      }
    }
    ...
    function drawMultiLineMarker(marker, range, clazz, config) {
      ...
      var style = markerEl && window.getComputedStyle(markerEl);
      if (style) {
