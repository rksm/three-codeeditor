;(function(exports) {
  ...
  function getRelativeMouseXY(x, y, domElement) {
    // Stolen from THREEx.DomEvents. Converts the browser global (page) x/y coordinates
    // into relative -1/1 values. These can be used by THREE for raycasting.

    var elPosition	= {x : 0 , y : 0}, elDimension;

    if (isFullscreen()) {
      elDimension	= { width	: window.innerWidth, height	: window.innerHeight };
      elPosition	= {x: 0, y:0};
    } else {
  	...
  	return convertXYToOcculusCoords({
  		x : +((x - elPosition.x) / elDimension.width ) * 2 - 1,
  		y : -((y - elPosition.y) / elDimension.height) * 2 + 1
  	});
  }
