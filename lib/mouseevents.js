;(function(exports) {
  ...
  function processScrollbarMouseEvent(THREExDOMEvents, codeEditor, clickState, evt3D) {
    ...
    // if (clickState.scrollbarClickPoint && evt3D.type === 'mousemove') {
    //   var yDiff = localBrowserPos.y - clickState.scrollbarClickPoint.y;
    ...
    var localBrowserPos = convertToLocalBrowserCoords(evt3D.intersect.point, codeEditor);
    var hit = codeEditor.scrollbar.left <= localBrowserPos.x && localBrowserPos.x <= codeEditor.scrollbar.left + codeEditor.scrollbar.width
           && codeEditor.scrollbar.top <= localBrowserPos.y && localBrowserPos.y <= codeEditor.scrollbar.top + codeEditor.scrollbar.height;
    ...
    codeEditor.scrollbar.backgroundColor = "red";
    ...
    function releaseScrollbar(evt) {
      evt.stopPropagation();
      codeEditor.scrollbar.backgroundColor = "green";
    ...
    // var scrollbar code
    // THREExDOMEvents, codeEditor, clickState, evt
  }
  ...
  function reemit3DMouseEvent(THREExDOMEvents, evt, clickState, codeEditor, globalPosForRealEditor) {
    ...
    else if (type === 'mousewheel' && aceEd.isFocused()) aceEd.$mouseHandler.onMouseWheel('mousewheel', fakeEvt);
  ...
  function patchEvent(evt, globalPosForRealEditor, aceEd) {
    ...
    if (evt.type === 'mousewheel') patchWheelEvent(evt, fakeEvt);
  ...
  function isLeftMouseButtonPressed(evt3D) { return evt3D.origDomEvent.which === 1 || evt3D.origDomEvent.buttons === 1; }
  function isRightMouseButtonPressed(evt3D) { return evt3D.origDomEvent.which === 3 || evt3D.origDomEvent.buttons === 2; }
